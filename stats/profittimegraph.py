...
if __name__ == '__main__':
	...
	for d in days:
		...
		results[None] = {k:v for k,v in bombers.items()}
		full = {i: (len(results[i]), sum(costs[d[0]] for d in results[i].values()), sum(v[1] for v in results[i].values())) for i in results}
		deadresults = {i: {k:v for k,v in results[i].items() if not v[2]} for i in results}
		dead = {i: (len(deadresults[i]), sum(costs[d[0]] for d in deadresults[i].values()), sum(v[1] for v in deadresults[i].values())) for i in results}
		if opts.opti:
