...
if __name__ == '__main__':
	...
	for fi,f in enumerate(hdata.Fighters.data):
		...
		if f['radpri']:
			...
			gr = plt.plot_date([datum['date'].ordinal() for datum in data if i(datum['date'])], [e['radar'][fi] for e in data if i(datum['date'])], fmt='o-', color=extra[f['name']]['2colour'], tz=None, xdate=True, ydate=False, label=None, zorder=0)
