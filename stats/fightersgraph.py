...
if __name__ == '__main__':
	...
	for fi,f in enumerate(hdata.Fighters.data):
		...
		if f['radpri']:
			gr = plt.plot_date([datum['date'].ordinal() for datum in data if hdata.inservice(datum['date'], f)], [e['radar'][fi] for e in data if hdata.inservice(e['date'], f)], fmt='o-', color=extra[f['name']]['2colour'], tz=None, xdate=True, ydate=False, label=None, zorder=0)
