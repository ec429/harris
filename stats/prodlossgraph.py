...
if __name__ == '__main__':
	...
	for bi,b in enumerate(hdata.Bombers.data):
		bprod = [shft(data[key][bi][0], bi) for key in sorted(data) if hdata.inservice(key, b)]
		bloss = [shft(data[key][bi][1], bi) for key in sorted(data) if hdata.inservice(key, b)]
	...
	for bi in range(32):
		bprod = [shft(data[key][bi+coff][0], bi) for key in sorted(data)]
		bloss = [shft(data[key][bi+coff][1], bi) for key in sorted(data)]
		if not any(bprod+bloss): continue
		bdate = [key.ordinal() for key in sorted(data)]
		gp = plt.plot_date(bdate, bprod, fmt='o-', mew=0, color='C%d'%bi, tz=None, xdate=True, ydate=False, label='Type %d'%(bi+coff), zorder=0)
