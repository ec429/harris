...
int raid_results_create(void)
{
	...
	else
	{
		...
		for(unsigned int i=0;i<ntypes;i++)
		{
			...
			if(atg_ebox_pack(RS_typerow, RS_typecol[i]))
			{
				perror("atg_ebox_pack");
				return(1);
			}
			...
			picture->w=38;
			if(atg_ebox_pack(RS_typecol[i], picture))
			{
				perror("atg_ebox_pack");
				return(1);
			}
			...
			if(atg_ebox_pack(RS_typecol[i], manu))
			{
				perror("atg_ebox_pack");
				return(1);
			}
			...
			if(atg_ebox_pack(RS_typecol[i], name))
			{
				perror("atg_ebox_pack");
				return(1);
			}
...
screen_id raid_results_screen(atg_canvas *canvas, game *state)
{
	if(totalraids)
	{
		atg_event e;
		// Raid Results table
		atg_box *b=raid_results_box->elemdata;
		for(unsigned int i=2;i<b->nelems;i++)
			atg_free_element(b->elems[i]);
		b->nelems=2;
