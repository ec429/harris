...
atg_element **HS_btrow, **HS_btcvt, **HS_btfrm;
char **HS_btnum;
...
atg_element **HS_flcr;
char *HS_flname, *HS_flest, *HS_flsta, *HS_flmark, *HS_flcond, **HS_flcc, **HS_flcrew;
...
int handle_squadrons_create(void)
{
	...
	if(!(HS_btfrm=calloc(ntypes, sizeof(*HS_btfrm))))
	{
		perror("calloc");
		return(1);
	}
	for(unsigned int i=0;i<ntypes;i++)
	{
		...
		HS_btfrm[i]=atg_create_element_button("Form squadron", (atg_colour){191, 191, 47, ATG_ALPHA_OPAQUE}, (atg_colour){47, 31, 31, ATG_ALPHA_OPAQUE});
		if(!HS_btfrm[i])
		{
			fprintf(stderr, "atg_create_element_button failed\n");
			return(1);
		}
		HS_btfrm[i]->hidden=true;
		if(atg_ebox_pack(vbox, HS_btfrm[i]))
		{
			perror("atg_ebox_pack");
			return(1);
		}
	...
	atg_element *flcdb=atg_create_element_box(ATG_BOX_PACK_HORIZONTAL, CREW_BG_COLOUR);
	if(!flcdb)
	{
		fprintf(stderr, "atg_create_element_box failed\n");
		return(1);
	}
	if(atg_ebox_pack(HS_fltbox, flcdb))
	{
		perror("atg_ebox_pack");
		return(1);
	}
	atg_element *flcol=atg_create_element_label("Condition:", 11, (atg_colour){191, 207, 191, ATG_ALPHA_OPAQUE});
	if(!flcol)
	{
		fprintf(stderr, "atg_create_element_label failed\n");
		return(1);
	...
	atg_element *flcdl=atg_create_element_label_nocopy(HS_flcond, 10, (atg_colour){191, 159, 127, ATG_ALPHA_OPAQUE});
	if(!flcdl)
	{
		fprintf(stderr, "atg_create_element_label failed\n");
		return(1);
	}
	if(atg_ebox_pack(flcdb, flcdl))
	{
		perror("atg_ebox_pack");
		return(1);
	}
...
void update_flt_info(game *state)
{
	...
	for(unsigned int i=0;i<state->nbombers;i++)
		if(state->bombers[i].squadron==selsqn && state->bombers[i].flight==selflt)
		{
			...
			int cond=max(100-state->bombers[i].wear, 0)/10;
			if(cond>=10)
				snprintf(HS_flcond+nb, 12-nb, "n");
			else
...
void update_sqn_info(game *state)
{
	...
	for(unsigned int i=0;i<ntypes;i++)
	{
		HS_btcvt[i]->hidden=prestart||selsqn<0||state->squads[selsqn].btype==i||active;
		HS_btfrm[i]->hidden=!prestart||selstn<0||bases[selstn].nsqns;
	}
				...
				case ATG_EV_TRIGGER:;
					...
					if(trigger.e==HS_split)
					{
						...
						(state->squads=n)[newsqn]=(squadron){
							...
							.rtime=state->squads[selsqn].rtime+(prestart?0:7),
							.third_flight=false,
					...
					for(i=0;i<ntypes;i++)
					{
						if(trigger.e==HS_btcvt[i])
						{
							...
							state->squads[selsqn].rtime+=prestart?0:related?14:types[i].heavy?28:21;
